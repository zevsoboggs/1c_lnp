...
/**
 * Тот же интерфейс, что давал useCustom, но ходит в наш бэкенд: eSIM живёт не
 * в admin-api (там модуль отключён), а у Yesim напрямую через /api/esim.
 */
function useEsim<T>(path: string, staleTime?: number) {
  const query = useQuery({
    queryKey: ['esim', path],
    queryFn: async () => {
      const r = await fetch(`/api/esim${path}`)
  ...
  return { result: { data: query.data }, query }
}
...
export const EsimPage = () => {
  ...
  const overview = useEsim<Overview>('/')
  const orders = useEsim<{ orders: Order[] }>('/orders')
  ...
  const plans = useEsim<{ plans: Plan[] }>('/plans', 10 * 60_000)
